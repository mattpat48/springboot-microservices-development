...
import it.disim.univaq.sose.examples.openjob.model.JobDomainStats;
...
public class JobController {
	...
	public ResponseEntity<Void> createJob(@RequestBody Job job) {
		...
		if (job.getCreatedBy() != null) {
			JsonNode userNode = userMicroserviceInvoker.findUserById(job.getCreatedBy());
			if (!hasRole(userNode, "job", "admin")) {
	...
	public ResponseEntity<Void> applytoJob(@PathVariable("username") String username, @PathVariable("id") Long id) {
		...
		if (jsonUser == null) {
			throw new NoSuchElementException("Utente candidato '" + username + "' non trovato nel sistema.");
		}
	...
	private boolean hasRole(JsonNode userNode, String... allowedRoles) {
		if (userNode == null) {
			return false;
		}
		JsonNode roles = userNode.get(UserMicroserviceInvoker.FIELD_ROLES);
		if (roles == null || !roles.isArray()) {
			return false;
		}
		for (JsonNode role : roles) {
			JsonNode nameNode = role.get(UserMicroserviceInvoker.FIELD_ROLE_NAME);
			if (nameNode != null) {
	...
	@GetMapping("/stats")
	public ResponseEntity<JobDomainStats> getStats() {
